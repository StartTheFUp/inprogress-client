export const reducer = (state, action) => {
  ...
  if (action.type === 'UPDATE_TODOS') {
    //const newBlock = state.blocks   state.blocks.filter(block=>block._id === action.idBlock.filter(block => block.sections.id === action.idSection).filter(block=>block.sections.element.id === action.idElement).filter(block=>block.sections.elements.id === action.idElement)
